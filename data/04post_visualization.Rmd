...
rmd_file <- glue("04post_visualization_{ver}.Rmd")
...
The hisat2-based alignments were passed to @kimGraphbasedGenomeAlignment2019, which
...
tarball, but I was concerned that I would need to change which files
are included, so did not.  Thus, if one wishes to play with the
following blocks, one will first need to run:
...
# Create a table of variants by gene

In the previous section, I did this process using older and newer data
(and disabled it because we do not have all of that data available for
the container; but I still want to do some of these comparisons and so
will do them using only the current/new tree.
...
# Print most variant genes

In my conversation today (20240614) with Maria Adelaida, she asked if
I can provide a table of genes with respect to number of variants and
differential expression.  It turns out that I basically already did;
the following block is pretty much exactly this query.  I think the
only thing I really need to consider is if I want to formalize this
idea so that I can use it elsewhere, or if it will be a one-off.
...
variants_wrt_logfc <- plot_scatter(vars_by_de_gene, xcol = "deseq_logfc",
                                   ycol = "var_len")
variants_wrt_logfc
...
```{r}
vars_by_sig_gene_idx <- abs(vars_by_de_gene[["deseq_logfc"]] >= 1.0) &
  vars_by_de_gene[["deseq_adjp"]] <= 0.05
vars_by_sig_gene <- vars_by_de_gene[vars_by_sig_gene_idx, ]
written <- write_xlsx(data = vars_by_sig_gene, excel = "excel/CDS_variants_by_sig_gene.xlsx")
```
...
fivep_vars_by_sig_gene_idx <- abs(fivep_vars_by_de_gene[["deseq_logfc"]] >= 1.0) &
  fivep_vars_by_de_gene[["deseq_adjp"]] <= 0.05
fivep_vars_by_sig_gene <- fivep_vars_by_de_gene[fivep_vars_by_sig_gene_idx, ]
written <- write_xlsx(data = fivep_vars_by_sig_gene, excel = "excel/fivep_variants_by_sig_gene.xlsx")

threep_vars_by_sig_gene_idx <- abs(threep_vars_by_de_gene[["deseq_logfc"]] >= 1.0) &
  threep_vars_by_de_gene[["deseq_adjp"]] <= 0.05
threep_vars_by_sig_gene <- threep_vars_by_de_gene[threep_vars_by_sig_gene_idx, ]
written <- write_xlsx(data = threep_vars_by_sig_gene, excel = "excel/threep_variants_by_sig_gene.xlsx")
```
